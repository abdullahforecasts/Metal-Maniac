...
struct Soldier {
    ...
    int life = 5;
    int EC = MaxEnemies;
    //bool isHit = false;   // NEW: Track if the enemy was recently hit
    //float hitTimer = 0;   // NEW: Timer for enemy reset
} sd;
...
void DrawEnemyBullets(Soldier enemy[]) {
    for (int i = 0; i < MaxEnemies; i++) {
...
void UpdateEnemies(Soldier& hero, Soldier enemy[]) {
    ...
    uniform_int_distribution<> range3(1, 300);

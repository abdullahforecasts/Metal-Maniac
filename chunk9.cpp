...
void InitGame(Soldier& hero, Soldier enemy[], Texture2D& bg1, Texture2D& bg2, int& bg1x, int& bg2x) {
    ...
    for (int i = 0; i < MaxEnemies; i++) {
        ...
        enemy[i].texture.width = PlayerWidth;
        enemy[i].texture.height = PlayerHeight;
        enemy[i].sdHeight = PlayerHeight;
        enemy[i].sdWidth = PlayerWidth;


        int random = rand() % 2;
        if (random == 0) {
            enemy[i].x = -PlayerWidth - (i + 1) * 5;
        }
        else {
            enemy[i].x = WindowWidth + (i + 1) * 5;
        }

        enemy[i].y = WindowHeight - hero.sdHeight - 140;
        enemy[i].hitbox.x = (float)(hero.x + 40);
        enemy[i].hitbox.y = (float)(hero.y + 15);
        enemy[i].hitbox.width = (float)(hero.sdWidth - 60);
...
void DrawGame(Soldier& hero, Soldier enemy[], Texture2D& bg1, Texture2D& bg2, int bg1x, int bg2x) {
    ...
    for (int i = 0; i < MaxEnemies; i++) {
        DrawTextureRec(enemy[i].texture, enemy[i].frameRec, { (float)enemy[i].x, (float)enemy[i].y }, WHITE);
        ...
        if (hero.bullets[i].active) {
            ...
            for (int j = 0; j < MaxEnemies; j++) {
                if (CheckCollisionCircleRec({ hero.bullets[i].x, hero.bullets[i].y }, hero.bullets[i].outer_radius, enemy[j].hitbox)) {
                    ...
                    int random = rand() % 2;
                    if (random == 0) {
                        enemy[j].x = (rand() % (-PlayerWidth - (-WindowWidth / 2))) + -WindowWidth / 2;
                    }
                    else {
                        enemy[j].x = (rand() % (3 * WindowWidth / 2 - WindowWidth)) + WindowWidth;
                    }
                }
            }
        }
...
void UpdateEnemies(Soldier& hero, Soldier enemy[]) {
    for (int i = 0; i < MaxEnemies; i++) {
        srand(GetFrameTime() * 1000 * (i + 1));
        if (CheckCollisionRecs(hero.hitbox, enemy[i].hitbox)) {
            int random = rand() % 2;
            if (random == 0) {
                enemy[i].x = (rand() % (-PlayerWidth - (-WindowWidth / 2))) + -WindowWidth / 2;
            }
            else {
                enemy[i].x = (rand() % (3 * WindowWidth / 2 - WindowWidth)) + WindowWidth;
            }
        }
        for (int j = 0; j < MaxBullets; j++) {
            if (CheckCollisionCircleRec({ hero.bullets[j].x, hero.bullets[j].y }, hero.bullets[j].outer_radius, enemy[i].hitbox)) {
                int random = rand() % 2;
                if (random == 0) {
                    enemy[i].x = (rand() % (-PlayerWidth - (-WindowWidth / 2))) + -WindowWidth / 2;
                }

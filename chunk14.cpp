...
void InitGame(Soldier& hero, Soldier enemy[], Texture2D& bg1, Texture2D& bg2, int& bg1x, int& bg2x, Jet& jet) {
    ...
    for (int i = 0; i < hero.MaxEnemies; i++) {
        ...
        enemy[i].enemyTexture[0].width = PlayerWidth + 20;
        enemy[i].enemyTexture[0].height = PlayerHeight - 20;
        enemy[i].enemyTexture[1] = LoadTexture("els.png");
        enemy[i].enemyTexture[1].width = PlayerWidth + 20;
        enemy[i].enemyTexture[1].height = PlayerHeight - 20;
        ...
        enemy[i].y = WindowHeight - hero.sdHeight - 130;
        enemy[i].hitbox.x = (float)(hero.x + 100);
        enemy[i].hitbox.y = (float)(hero.y - 10);
    ...
    jet.x = -JetWidth;  // Start from the left
    jet.y = 100; // Mid-air position
    jet.speed = 3; // Adjust speed if necessary

    for (int i = 0; i < MaxJetBullets; i++) {
        jet.bullets[i] = { 0.0f, 0.0f, 5.0f, false, 1 };  // Initialize bullets
    }
...
void DropJetBullets(Jet& jet) {
    ...
    static std::uniform_int_distribution<> dropChance(1, 100);
